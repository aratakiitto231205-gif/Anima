        ...
        if (valEl && barEl) {
            ...
            if (k === 'energy') {
                barEl.style.background =
                    val < 3.0 ? 'linear-gradient(90deg, #ef4444, #f87171)' : 'linear-gradient(90deg, #10b981, #34d399)';
...
export function renderBeliefs(agent, saveAgentStateFn, activeEnvironment) {
    ...
    if (beliefsListEl) {
        if (agent.memory.beliefs && agent.memory.beliefs.length > 0) {
            beliefsListEl.innerHTML = agent.memory.beliefs
                .map(
                    (b) =>
...
export function renderMemories(agent) {
    ...
    if (drawerListEl) {
        if (agent.memory.recallable_drawer && agent.memory.recallable_drawer.length > 0) {
            drawerListEl.innerHTML = agent.memory.recallable_drawer
                .map((c) => {
...
export function renderTriggers(agent) {
    ...
    if (triggersListEl) {
        if (agent.biomarker_triggers && agent.biomarker_triggers.length > 0) {
            triggersListEl.innerHTML = agent.biomarker_triggers
                .map(
                    (t) =>
            ...
            if (items.length > 0) {
                itemsEl.innerHTML = items
                    .map((item) => {
                        const qtyBadge =
                            item.quantity > 1
                                ? `<span style="background: rgba(16, 185, 129, 0.15); color: #34d399; padding: 1px 4px; border-radius: 3px; font-size: 0.85em; font-weight: bold; margin-left: 4px;">x${item.quantity}</span>`
                                : '';
...
export function updateActiveRecallUI(activeRecalledMemories) {
    ...
    if (activeRecalledMemories && activeRecalledMemories.length > 0) {
        activeRecallEl.innerHTML = activeRecalledMemories
            .map(
                (m) =>
