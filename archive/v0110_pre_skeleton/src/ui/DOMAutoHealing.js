...
export async function renderParsedMessage(
    ...
    // 3. Render to DOM
    setTimeout(() => renderToDom(messageId, rawText, parsed), TIMING.DOM_RENDER_DELAY_MS);
}
...
let chatObserverRetries = 0; // Bug 3: bounded retry counter
...
export function startChatObserver(getActiveAgentFn, saveAgentStateFn, refreshUIFn) {
    ...
    if (!chatEl) {
        // Bug 3: give up after OBSERVER_MAX_RETRIES to prevent infinite polling on broken ST
        if (chatObserverRetries++ >= TIMING.OBSERVER_MAX_RETRIES) {
            console.error(
                `Anima Engine: #chat not found after ${TIMING.OBSERVER_MAX_RETRIES} retries (${TIMING.OBSERVER_MAX_RETRIES * TIMING.OBSERVER_RETRY_DELAY_MS}ms), giving up`
            );
            chatObserverRetries = 0;
            return;
        }
        setTimeout(() => startChatObserver(getActiveAgentFn, saveAgentStateFn, refreshUIFn), TIMING.OBSERVER_RETRY_DELAY_MS);
        return;
    }
    chatObserverRetries = 0; // reset on success
        ...
        if (needsRender) {
            ...
            messages.forEach((messageEl) => {
                ...
                if (!hasVnStyle && msgObj && msgObj.mes) {
                    ...
                    }, TIMING.OBSERVER_REMOUNT_DELAY_MS);
                }
            });
        }
    });
