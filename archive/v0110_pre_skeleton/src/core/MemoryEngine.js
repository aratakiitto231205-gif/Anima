...
export class MemoryEngine {
     ...
     * Khi user scrub/edit lại message cũ, hệ thống rollback hormone về state tương ứng
     * trong neuro_history (nếu có). Ký ức song song được bảo toàn để tạo cảm giác
     * "déjà vu" — character có ký ức tương lai nhưng hormone quay về quá khứ.
     * Bản nâng cấp v8.1: KHÔNG XÓA KÝ ỨC TƯƠNG LAI. Future neuro state tự nhiên
     * bị shadow bởi past state cho đến khi user gen lại.
     */
    applyTemporalAnchor(currentMessageIndex, hormones = null, neuro_history = null) {
        ...
        if (hormones && neuro_history) {
            ...
            const indices = Object.keys(neuro_history)
                .map(Number)
                .sort((a, b) => b - a);
    ...
    decayShortTermMemory(elapsedMinutes) {
        ...
        this.stm_buffer = this.stm_buffer
            .map((mem) => {
    ...
    learnMemoryDynamically(text, messageIndex, hormones) {
        ...
        const newId = 'mem_' + (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : Date.now() + '_' + Math.random().toString(36).substr(2, 5));
...
export function findNewestMemory(agent) {
    ...
    const all = [...(agent.memory.recallable_drawer || []), ...(agent.memory.stm_buffer || [])];
    if (all.length === 0) return null;
    return all.reduce((newest, m) => (new Date(m.timestamp) > new Date(newest.timestamp) ? m : newest));
}
