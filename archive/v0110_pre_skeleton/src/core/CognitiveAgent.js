...
export class CognitiveAgent {
    ...
    updateVitalsAndSensations(updates) {
        ...
        for (const [key, val] of Object.entries(updates)) {
            ...
            if (this.body_status[k] !== undefined) {
                ...
                // Bug 10: warn about unknown keys so LLM typos don't silently fail
                logAnima('warning', 'Agent', `updateVitalsAndSensations: unknown key "${k}"`);
            }
        }
    ...
    processMessage(text, role, currentMessageIndex) {
        ...
        // Bug 16: removed dead return [] (never used by callers)
    }
