...
export class HormoneEngine {
    ...
    decay(elapsedMinutes, bodyState = '', genetics = null) {
        ...
        const isExhausted =
            bodyStr.includes('kiệt sức') ||
            bodyStr.includes('mệt') ||
            bodyStr.includes('chấn thương') ||
            bodyStr.includes('yếu');
        ...
        if (genetics) {
            if (genetics.comt === 'Val/Val')
                comtMultiplier = 4.0; // Warrior phân hủy dopamine nhanh gấp 4 lần
    ...
    evaluateEvent(semanticTags, genetics = null) {
        ...
        if (semanticTags.has('#danger')) {
            this.levels.adrenaline = applyHillEquation(
                this.levels.adrenaline,
                1.5 * scale * getJitterMult(),
                genetics,
                'adrenaline'
            );
            this.levels.cortisol = applyHillEquation(
                this.levels.cortisol,
                0.6 * scale * getJitterMult(),
                genetics,
                'cortisol'
            );
            this.levels.melatonin = applyHillEquation(
                this.levels.melatonin,
                -1.0 * scale * getJitterMult(),
                genetics,
                'melatonin'
            );
            changed = true;
        ...
        if (semanticTags.has('#intimate')) {
            this.levels.oxytocin = applyHillEquation(
                this.levels.oxytocin,
                1.5 * scale * getJitterMult(),
                genetics,
                'oxytocin'
            );
            this.levels.serotonin = applyHillEquation(
                this.levels.serotonin,
                1.2 * scale * getJitterMult(),
                genetics,
                'serotonin'
            );
            this.levels.dopamine = applyHillEquation(
                this.levels.dopamine,
                0.8 * scale * getJitterMult(),
                genetics,
                'dopamine'
            );
            this.levels.cortisol = applyHillEquation(
        ...
        if (semanticTags.has('#laugh')) {
            this.levels.endorphins = applyHillEquation(
                this.levels.endorphins,
                1.5 * scale * getJitterMult(),
                genetics,
                'endorphins'
            );
        ...
        if (semanticTags.has('#flirt')) {
            this.levels.sex_hormones = applyHillEquation(
                this.levels.sex_hormones,
                1.2 * scale * getJitterMult(),
                genetics,
                'sex_hormones'
            );
