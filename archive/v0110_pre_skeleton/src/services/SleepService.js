...
async function callDreamLLM(agent, sleepDurationMinutes, wasInterrupted, characterName) {
    ...
    try {
        ...
        logAnima('error', 'Sleep', `Sleep LLM consolidation failed, falling back: ${err.message}`);
        return null;
    }
}
        ...
        for (const content of consolidated) {
            const card = {
                id: typeof crypto !== 'undefined' && crypto.randomUUID
                    ...
                    : 'mem_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5),
                content,
